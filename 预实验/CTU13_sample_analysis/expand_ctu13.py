"""CTU-13 全量提取: 解析 6 个 pcap 的全部数据包 -> ctu13_protocol_headers_full.csv
原 process_remaining.py 每文件只取前 5000 包 (MAX_PKTS=5000, 共 30,000), 导致样本过小
本脚本用 dpkt (快) 全量解析, 字段语义与 scapy 版完全一致
用法: python expand_ctu13.py [LIMIT]   # LIMIT 缺省 0 = 全部; >0 = 每文件只取前 LIMIT 包 (测试用)
...
def parse_pkt(pkt, tstamp):
    f = {}
    try:
        eth = pkt
        if not isinstance(eth, dpkt.ethernet.Ethernet):
            raise ValueError('not ethernet')
...
print(f"{len(pcaps)} pcap files, LIMIT={LIMIT}")
t0 = datetime.now()
total = 0; errs = 0
        ...
        try:
            with open(pp, 'rb') as fh:
                for ts, buf in dpkt.pcap.Reader(fh):
                    ...
                    try:
                        row = parse_pkt(dpkt.ethernet.Ethernet(buf), ts)
                    except Exception:
                        errs += 1; continue
                    row['label'] = pp.stem; row['src_file'] = pp.name
                    w.writerow(row); n += 1
                    if n % 200000 == 0:
                        print(f"  {pp.name}: {n:,} pkts, {total+n:,} total, {(datetime.now()-t0).total_seconds():.0f}s")
        except Exception as e:
            print(f"  {pp.name}: ERROR {e}")
        total += n
        print(f"  {pp.name}: {n:,} pkts ({(datetime.now()-t0).total_seconds():.0f}s)")

...
print(f"   DNN Acc={dnn_acc*100:.4f}% Prec={dnn_p*100:.4f}% Recall={dnn_r*100:.4f}% F1={dnn_f1*100:.4f}%")
...
def make_pareto_plot(items, total, fname):
    ...
    max_x = n
    if max_x > 10000:
        ticks = np.linspace(0, max_x, 8)
        ax.set_xticks(ticks)
        ax.set_xticklabels([f'{int(x/1000)}K' if x>=1000 else f'{int(x)}' for x in ticks])
